...
func TestComputeRecoveryTimeIncludesSmallDeferredQueues(t *testing.T) {
	start := time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC)
	points := []Point{
		{Timestamp: start, NetGridMW: 0, FrequencyHz: 60, VoltagePU: 1},
		{Timestamp: start.Add(time.Minute), NetGridMW: 1, DeferredQueueMWh: 0.0005, FrequencyHz: 60, VoltagePU: 1},
		{Timestamp: start.Add(3 * time.Minute), NetGridMW: 2, DeferredQueueMWh: 0, FrequencyHz: 60, VoltagePU: 1},
	}
	cfg := scenario.Config{Controller: scenario.ControllerConfig{RampLimitMWPerMin: 5, RecoveryWindow: "30m", EventFrequencyLowHz: 59.95, EventVoltageLowPU: 0.95}}
	summary := Compute(points, points, cfg, pmu.EventSummary{})
	if summary.WorkloadRecoveryTimeSeconds != 120 {
		t.Fatalf("expected small queue recovery at 120s, got %+v", summary)
	}
}
